...
**Traits**: Concrete file paths + line numbers; tiered code detail — full code for contract/non-obvious files, skeleton + pattern-reference for boilerplate; mirrors existing patterns; invokes whatever skills the project makes available before first-principles design; discovers and records the project's real Gradle verification commands in Section 0 rather than assuming defaults; annotates every interactive UI element with a stable `testTag`/`contentDescription` and emits a UI Selectors table; discovers the project's test stack and names the unit tests each section needs — meaningful cases or an explicit `None — <reason>`, never fluff; authors the Manual Testing Plan with selector-annotated steps the Tester drives directly.
...
**Traits**: Verifies the plan's file/type/API claims against the real codebase; checks completeness, Section 0's Gradle tasks against the project's actual build files, pattern-fit, the parallel-safety call, and test-plan coverage — accepting a well-reasoned `N/A` as complete rather than demanding a case for a category the feature can't exercise; decisive — blocks only where a Coder would guess or hit a contradiction, not on style.
