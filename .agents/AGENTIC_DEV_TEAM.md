...
**Traits**: Concrete file paths + line numbers; tiered code detail — full code for contract/non-obvious files, skeleton + pattern-reference for boilerplate; mirrors existing patterns; invokes whatever skills the project makes available before first-principles design; discovers and records the project's real Gradle verification commands in Section 0 rather than assuming defaults; annotates every interactive UI element with a stable `testTag`/`contentDescription` and emits a UI Selectors table; authors the Manual Testing Plan with selector-annotated steps the Tester drives directly.
